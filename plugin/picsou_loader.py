...
class PicsouLoadQuotes():
            ...
            for cours in courss:
                ...
                if not btraite and b_en_test:
                    ...
                    if b_vendre:
                        ...
                        if gain > 0:
                            self.parent.display("  {:8.2f}€\t{:7.2f}€\t\t{} / {} {:.0f}/{:.0f}\t{:3.0f}j {}"\
                            .format(cumul, gain, date_achat, cours["cours_date"], rsi_achat, rsis[0], nbj, motif))
                        else:
                            self.parent.display("  {:8.2f}€\t\t{:7.2f}€\t{} / {} {:.0f}/{:.0f}\t{:3.0f}j {}"\
                            .format(cumul, gain, date_achat, cours["cours_date"], rsi_achat, rsis[0], nbj, motif))
    ...
    def account(self):
        ...
        mvts = self.crud.sql_to_dict(self.crud.get_basename(), """
        SELECT * FROM mvt
        WHERE mvt_select = '1' and mvt_account not like '%SIMUL%'
        ORDER BY mvt_ptf_id, mvt_date asc
        """, {})
        ptf_id = ""
        accounts = {}
...
class PicsouLoadQuotesJ():
    ...
    def run(self, ptf_id, nbj):
        ...
        self.crud.exec_sql(self.crud.get_basename(), """
        insert into cdays
        (cdays_ptf_id, cdays_name, cdays_date, cdays_time, cdays_close
        , cdays_open, cdays_volume, cdays_low, cdays_high)
        select id, name, date, CURRENT_TIMESTAMP, close, open, volume, low, high 
        from quotes
        left outer join cdays on cdays_ptf_id = quotes.id and cdays_date = quotes.date
        where cdays_ptf_id is null and quotes.id = :id and quotes.date = date('now')
        """, {"id": ptf_id})
        # calcul cours_percent
        self.crud.exec_sql(self.crud.get_basename(), """
        UPDATE cdays
    ...
    def csv_to_quotes(self, ptf, nbj):
        """
        Récupération du dernier cours d'une action
        """
        # end_date = int(time.mktime(datetime.datetime.now().timetuple()))
        end_date = int(time.time())
